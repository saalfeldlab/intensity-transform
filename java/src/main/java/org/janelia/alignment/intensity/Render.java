...
public class Render
{
			...
			final double scale )
	{
		...
		affineScale.set( ( float )scale, 0, 0, ( float )scale, -( float )( x * scale ), -( float )( y * scale ) );
		ctl.add( affineScale );
		...
		int mipmapLevel = bestMipmapLevel( s );	
		...
		/* convert */
		final float min = ( float )patch.getMin();
		final float max = ( float )patch.getMax();
		final float a = 1.0f / ( max - min );
		final float b = 1.0f / 255.0f;
		
		for ( int i = 0; i < alphaPixels.length; ++i )
			targetImage.setf( i, ( tp.getf( i ) - min ) * a );
		
		for ( int i = 0; i < alphaPixels.length; ++i )
			targetWeight.setf( i, ( alphaPixels[ i ] & 0xff ) * b );
	}
