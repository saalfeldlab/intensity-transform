...
//		run( project );
	...
	final static public < M extends Model< M > & Affine1D< M > >void run(
...
//		final PointMatchFilter filter = new RansacRegressionFilter();
		...
		/* single layer */
//		final Layer layer = project.getRootLayerSet().getLayer( 10 );
//		@SuppressWarnings( { "rawtypes", "unchecked" } )
//		final ArrayList< Patch > patches = ( ArrayList )layer.getDisplayables( Patch.class );
		...
		for ( final Patch p1 : patches )
		{
			...
			for ( int i = layerIndex - neighborhoodWidth; i <= layerIndex + neighborhoodWidth; ++i )
			{
			...
			/* within just a single layer */
//			p2s.addAll( ( Collection )p1.getLayer().getDisplayables( Patch.class, box1 ) );
...
//					IJ.log( "bang" );
		...
		try
		{
			tc.optimize( 0.01f, 2000, 2000, 0.75f );
		}
...
//		final Project project = Project.openFSProject( "/data/saalfeld/bocklab/autoloader/untitled.elastic.xml", false );
